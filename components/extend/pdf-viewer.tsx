...
function PDFViewerFallbackShell({
  ...
  return (
    <div
      ...
      className={cn(
        "flex h-full max-h-full min-h-0 w-full flex-col overflow-hidden bg-slate-100 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100",
        className
      )}
    >
      {showToolbar ? (
        <PDFViewerToolbar
          activePage={1}
          controlsDisabled
          currentZoomLevel={typeof defaultZoom === "number" ? defaultZoom : 1}
          numPages={0}
          searchControl={
      ...
      <div className="relative flex min-h-0 flex-1 overflow-hidden bg-slate-200/70 dark:bg-zinc-950">
        {state === "loading" ? (
...
function PDFViewerToolbar({
  ...
  return (
    <div className="flex min-h-12 w-full max-w-full items-center justify-between gap-1.5 border-b border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 text-slate-800 dark:text-zinc-100 px-2.5 py-1.5 overflow-x-auto overflow-y-hidden shrink-0 select-none">
      ...
      <TooltipProvider>
        <div className="flex min-w-0 items-center justify-end gap-1 shrink-0">
          {showRotateControls ? (
            <>
...
function PDFViewerInner({
  ...
  React.useEffect(() => {
    ...
    zoom.requestZoom(defaultZoom as any)
  ...
  return (
    ...
    >
      ...
      <div
        ref={viewerShellRef}
        className="relative flex min-h-0 flex-1 overflow-hidden bg-slate-200/70 dark:bg-zinc-950"
      >
        {isLoading ? (
  ...
  function PDFViewer(
    ...
    const [plugins] = React.useState(() => [
      ...
      createPluginRegistration(ZoomPluginPackage, {
        defaultZoomLevel: defaultZoom as any,
